...
export class DocumentReviewComponent extends Component {
    ...
    async _uploadSingleFile(file) {
        ...
        try {
            ...
            if (attachment.error) {
                this.notification.add(_t(attachment.error), {type: "warning", sticky: true});
            }
            attachment.state = "pending";
            this.state.newReview.attachments.push(attachment);
        } catch (error) {
            if (error instanceof RPCError) {
                this.notification.add(_t("Could not save file <strong>%s</strong>", escape(file.name)), {
                    type: "warning",
                    sticky: true,
                });
            }
    ...
    async removeAttachment(attachmentIndex) {
        ...
        try {
            ...
            this.notification.add(_t("Could not remove attachment %s", attachment.name), {type: "warning"});
        } finally {
            this.state.loading = false;
        }
    }
    ...
    canReplyToReview(review) {
        ...
        const userReplies = review.replies.filter((reply) => reply.author_name === this.currentPartnerName);
    ...
    getFileIcon(mimetype, filename = "") {
        ...
        if (mimetype.includes("excel") || mimetype.includes("spreadsheetml") || /\.(xls|xlsx)$/i.test(filename)) {
            return "fa-file-excel-o";
        }
        if (mimetype.includes("powerpoint") || mimetype.includes("presentationml") || /\.(ppt|pptx)$/i.test(filename)) {
